function exibeListaDePostagens() {
      ...
      .then(postagens => {
        ...
        for (let postagem of postagens) {
          ...
          for (let i = 0; i < campos.length; i++) {
            ...
            let texto = document.createTextNode(postagem[campos[i]]) as Text;
  ...
  function exibeComentariosDaPostagem(postagemId: any) {
    fetch(backendAddress + `forum/comentarios/${postagemId}`)
      ...
      .then(comentarios => {
        ...
        for (let comentario of comentarios) {
          ...
          for (let i = 0; i < campos.length; i++) {
            let td = document.createElement('td') as HTMLTableCellElement;
            let texto = document.createTextNode(comentario[campos[i]]) as Text;
            td.appendChild(texto);
            tr.appendChild(td);
          }
          tbody.appendChild(tr);
  ...
  function exibePostagensEComentariosDoForum(forumId: any) {
    ...
    fetch(backendAddress + "forum/lista/")
      ...
      .then(forums => {
        ...
        for (let forum of forums) {
          ...
          for (let i = 0; i < campos.length; i++) {
            let td = document.createElement('td') as HTMLTableCellElement;
            let texto = document.createTextNode(forum[campos[i]]) as Text;
            td.appendChild(texto);
            tr.appendChild(td);
          }
